...
The current action profile is 1.0.0; the current tools are 1.1.0. The specification, JSON Schemas, CLI, and supporting services are available for direct use. The [governance guide](GOVERNANCE.md) describes version compatibility and changes.
...
Yes. Install [ruagentic from npm](https://www.npmjs.com/package/ruagentic) with `npm install -g ruagentic@1.1.0`. The same package is available from [GitHub releases](https://github.com/sam1siam/agentic/releases/tag/v1.1.0). Use [Get started](GETTING-STARTED.md) for generation and validation commands. Node 24 or newer is required.
