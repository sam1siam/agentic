...
The same audit is available through the hosted and local MCP tools. See [Connect](https://ruagentic.org/connect/) for setup. To work entirely locally, use `agentic validate agentic.json openapi.json` and `agentic text agentic.json --check`.
