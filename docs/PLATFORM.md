...
The auditor reads a supplied HTTPS profile URL, same-origin OpenAPI bindings, and agents.txt, agents.json, and llms.txt. It reports content types, sizes, and status codes. It makes no action submissions and does not prove runtime idempotency, authorization, retention, or recovery behavior.
